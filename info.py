...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '1002139786268', '-100209901165').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL','-1002286374748')
...
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/+JiVi4by2Aag3MzE1')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/+kOcXJRcETpVjMWVl')
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/+kOcXJRcETpVjMWVl')
IS_TUTORIAL = bool(environ.get('IS_TUTORIAL', True))
MSG_ALRT = environ.get('MSG_ALRT', 'Wʜᴀᴛ Aʀᴇ Yᴏᴜ Lᴏᴏᴋɪɴɢ Aᴛ ?')
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002483919278'))
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', ''@t.me/+sLdUkZ4lhlU2NWNl')
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-100209901165' '-1002139786268')).split()]
